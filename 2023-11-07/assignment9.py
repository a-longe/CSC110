...
def read_file(filename: str) -> (dict[str, Date],
    ...
    id_date:dict[str:Date] = {}
    id_cast:dict[str:list[str]] = {}
    catagory_id:dict[str:list[str]] = {}
    ...
    with open(filename, 'r') as file_handler:
        ...
        for line in file_handler:
            line = line.strip()
            show_id, show_type, title, director_str, cast_str, country, \
            date_added_str, release_year, rating, duration, listed_in_str, \
            description = line.split(',')
            directors = director_str.split(':')
            listed_in = listed_in_str.split(':')
            cast = cast_str.split(':')
            date_added = create_date(date_added_str)
            id_date[show_id] = date_added
            id_cast[show_id] = cast
            id_title[show_id] = title
            for catagory in listed_in:
                if catagory in catagory_id.keys():
                    catagory_id[catagory].append(show_id)
                else:
                    catagory_id[catagory] = [show_id]
    # print(id_date, '\n')
    # print(id_cast, '\n')
    # print(catagory_id, '\n')
    # print(id_title, '\n')
    return (id_date, id_cast, catagory_id, id_title)
