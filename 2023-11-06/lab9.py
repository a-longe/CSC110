...
# repersents the tuple returned by dict.items()
KeyValuePair = tuple[int, int]
KEY = 0
VALUE = 1
...
def count_evens_odds(filename: str) -> dict[str, int]:
    ...
    {'Even': 588, 'Odd': 565}
    """
    even_odd_dict:dict[str:int] = {
        'Even':0,
        'Odd':0
    }
    if not path.exists(filename): return even_odd_dict
    with open(filename, 'r') as file_handler:
        for number_str in file_handler:
            num = int(number_str)
            if num % 2 == 0:
                # num is even
...
474: 1, 625: 1, 577: 1, 414: 1, 19: 1}
    """
    num_freq = {}
    if not path.exists(filename): return num_freq
    with open(filename, 'r') as file_handler:
        for number_str in file_handler:
            num = int(number_str)
...
def most_freq_number(filename: str) -> list[int]:
    ...
    num_freq = get_number_frequencies(filename)

    highest_frequency = 0
    most_freq_num: list[int] = []
    for dict_item in num_freq.items():
        if dict_item[VALUE] > highest_frequency:
            highest_frequency = dict_item[VALUE]
            most_freq_num = [dict_item[KEY]]
        elif dict_item[VALUE] == highest_frequency:
            most_freq_num.append(dict_item[KEY])
    return sorted(most_freq_num)
...
37: ['Liechtenstein']})
    ...
    if not path.exists(filename): return (country_pop, pop_country)
    with open(filename, 'r') as file_handler:
        for country in file_handler:
            country = country.strip()
            country_name, pop = country.split(',')
            pop = int(pop)

            country_pop[country_name] = pop

            pop_key = pop // granularity
            if pop_key in pop_country.keys():
                pop_country[pop_key].append(country_name)
            else:
                pop_country[pop_key] = [country_name]

    return (country_pop, pop_country)
...
(2876101, 'Albania'), (2881355, 'Jamaica'), (2924816, 'Armenia')]
    """
    country_pop, pop_country = create_population_dictionaries(filename,
                                                              granularity)
    # find countries in the same list as the given country
    try: pop = country_pop[country]
    except KeyError: return []
    pop_bin_key = pop // granularity
    similar_pop_countries:list[str] = pop_country[pop_bin_key]
    
    
    unsorted_analysis_results:list[PopulationInfo] = []
    for country in similar_pop_countries:
        population = country_pop[country]
        unsorted_analysis_results.append((population, country))

    # sort
    analysis_results = []
    try: analysis_results.append(unsorted_analysis_results[0])
    except IndexError: return []
    for unsorted_population_info in unsorted_analysis_results[1:]:
        has_inserted = False
        for index, sorted_population_info in enumerate(analysis_results):
            if unsorted_population_info[POPULATION] < \
            sorted_population_info[POPULATION]:
                analysis_results.insert(index, unsorted_population_info)
                has_inserted = True
                break
        if not has_inserted: analysis_results.append(unsorted_population_info)
